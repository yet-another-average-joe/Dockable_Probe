...
class DockableProbe:
    def __init__(self, config):
        ...
        self.attach_step_z = config.getfloat('attach_step_z', 1.0, above=0.)
...
# Y@@J #########################################################################################################
    ...
    def attach_probe(self, return_pos=None):
        ...
        # /!\ UNSAFE : attach or die in an infinite loop while crashing into the dock /!\
        # TODO : crash detecting dock -> 3D printed mechanical design :
        # similar to the Voron Unclicky TAP with 6x3 mm magnets acting as a spring and a NO switch

        # attach :
        while self.get_probe_state() != PROBE_ATTACHED :
            tpos = self.toolhead.get_position()
            self.toolhead.set_position([tpos[0], tpos[1], self.attach_step_z, tpos[3]], homing_axes=[2])
            self.toolhead.manual_move([None, None, -self.attach_step_z], self.attach_speed)
            self.toolhead.set_position([tpos[0], tpos[1], tpos[1]-self.attach_step_z, tpos[3]], homing_axes=[2])

        # extract :
        tpos = self.toolhead.get_position()
        self.toolhead.set_position([tpos[0], tpos[1], 0, tpos[3]], homing_axes=[2])
        self.toolhead.manual_move([None, None, self.z_hop], self.lift_speed)
        self.toolhead.set_position([tpos[0], tpos[1], tpos[2]+self.z_hop, tpos[3]], homing_axes=[2])

        # callback, for more moves if required
        self.gcode.run_script_from_command("""
            MOVE_TO_EXTRACT_PROBE
        """)
